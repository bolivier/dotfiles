  ...
  (let [get-device-data
        (fn [line]
          (let [[_ id] (str/split line #" " 3)
                base (->> (p/shell {:out :string} (str "bluetoothctl info " id))
                          :out
                          str/split-lines
                          (map (fn [line]
                                 (when-let [[_ k v] (re-find #"\t(\w+): (.*)" line)]
                                   [k v])))
                          (remove nil?)
                          (into {}))]
            (assoc base "ID" id)))]
...
(defn connected? [device]
  (= "yes" (get device "Connected")))

(defn headset? [device]
  (= "audio-headset" (get device "Icon")))
...
(do
  (case (first *command-line-args*)
    ...
    "disconnect" (do (p/shell "bluetoothctl disconnect " (second *command-line-args*))
